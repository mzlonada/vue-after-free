...
function show_success(immediate) {
  if (immediate) {
...
var themeFolder = typeof CONFIG !== 'undefined' && typeof CONFIG.theme === 'string' ? CONFIG.theme : 'default';
...
if (!is_jailbroken) {
  ...
  if (jb_behavior === 1) {
    ...
    if (compare_version(FW_VERSION, '7.00') >= 0 && compare_version(FW_VERSION, '12.02') <= 0) {
      use_lapse = true;
      lapse();
    } else if (compare_version(FW_VERSION, '12.50') >= 0 && compare_version(FW_VERSION, '13.04') <= 0) {
      include('netctrl_c0w_twins.js');
    }
  }
  ...
  try {
    include('themes/' + themeFolder + '/main.js');
  } catch (e) {/* escaped sandbox */}
}
