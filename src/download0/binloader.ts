...
function binloader_init() {
  ...
  '/mnt/usb3/payload.bin.bin', '/mnt/usb4/payload.bin.bin', '/mnt/sandbox/download/CUSA00960/payload.bin'];
    ...
    run: function () {
      ...
      if (ret.eq(0)) {
        ...
        if (typeof CONFIG !== 'undefined' && CONFIG.autoclose && !BinLoader.skip_autoclose) {
          var closeDelay = typeof CONFIG !== 'undefined' && CONFIG.autoclose_delay ? CONFIG.autoclose_delay : 0; // set to 20000 for ps4 hen
  ...
  function bl_load_from_file(path) {
    ...
    try {
      ...
      if (e.stack) log((_stack = e.stack) !== null && _stack !== void 0 ? _stack : '');
      return false;
    }
    return true;
  }
  ...
  function bl_network_loader() {
    ...
    try {
      ...
      var _stack2;
      log('ERROR loading payload: ' + e.message);
      if (e.stack) log((_stack2 = e.stack) !== null && _stack2 !== void 0 ? _stack2 : '');
      return false;
    }
    return true;
  }
